# XRP Chess — Terms of Service

**Last updated:** April 2026

By using the XRP Chess bot, you agree to the following terms.

## Description

XRP Chess is a free Discord chess bot featuring NFT art from the XRP Chess collection on the XRP Ledger. The bot provides chess gameplay, learning tools, tournaments, and cosmetic piece skins.

## Free to Use

XRP Chess is completely free. There are no paid features, subscriptions, premium tiers, or in-app purchases. All 394 piece skins are available to every player at no cost and require no NFT ownership.

## Fair Play

- The bot includes an AI anti-cheat system that monitors PvP and tournament games
- Using external chess engines, analysis tools, or assistance during games is prohibited
- Players caught cheating may receive flags on their profile or be permanently blacklisted
- Server admins can manage anti-cheat actions through the Admin Portal

## User Conduct

- Use the bot in accordance with Discord's Terms of Service and Community Guidelines
- Do not attempt to exploit, abuse, or disrupt the bot's functionality
- Do not spam commands or intentionally trigger rate limits
- Treat other players with respect during games and in chat

## NFT Art

- The 394 piece skins displayed in the bot are artwork from the XRP Chess NFT collection
- Piece skins are free cosmetic content for all users — no ownership or purchase is required
- The art is displayed for entertainment purposes within the Discord bot

## Availability

- XRP Chess is provided as-is with no guarantees of uptime or availability
- The bot may be updated, modified, or taken offline at any time
- Game data, ratings, and statistics may be reset during major updates

## Limitation of Liability

XRP Chess and its creators (Noble Apprentice & MadKilla710) are not liable for any damages, losses, or disputes arising from use of the bot. The bot is a free entertainment tool provided without warranty.

## Changes

These terms may be updated from time to time. Continued use of the bot constitutes acceptance of the current terms.

## Contact

- Discord: Noble Apprentice (nobleapprentice.)
- Server: https://discord.gg/vwA33NRBNp
- Twitter/X: https://x.com/XRPCHESS
